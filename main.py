...
def get_args_parser():
    ...
    parser.add_argument('--fusion_modality', default='all',
                        ...
                        help='Modality combination for ablation: all (default), rgb_ir, rgb_depth, rgb_only')
...
def main(args):
    ...
    # [新增] 三模态分层学习率
    if getattr(args, 'param_dict_type', 'default') == 'multimodal':
        from util.get_param_dicts import get_multimodal_param_dict
        param_dicts = get_multimodal_param_dict(args, model_without_ddp)
        # multimodal 模式下 lr 参数已分组，AdamW 的默认 lr 不再使用
    ...
    data_loader_train = DataLoader(dataset_train, batch_sampler=batch_sampler_train,
                                   collate_fn=utils.collate_fn_multimodal if args.dataset_file == 'multimodal' else utils.collate_fn,
    ...
    elif args.dataset_file == 'multimodal':
        # [新增] 三模态数据集不使用 COCO 格式的 CocoEvaluator
        # 评估时仅计算 loss，不计算 mAP（后续阶段六实现自定义评估）
        base_ds = None
    else:
