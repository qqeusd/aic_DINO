...
def build_ir_encoder(args):
    """
    构建红外 (IR) 模态的轻量编码器。

    从 LightweightEncoder 创建，输入 3 通道（RGB 兼容），
    输出 256 维特征图，空间尺寸为输入的 1/4。

    注意：虽然红外本质是单通道灰度，但保留 3 通道以复用
    RGB 的预处理流程和 ImageNet 预训练统计量。

    Args:
        args: 全局配置，目前 IR 编码器不依赖额外参数。

    Returns:
        LightweightEncoder: 红外编码器实例。
    """
    from models.multimodal_encoders import LightweightEncoder
    return LightweightEncoder(in_channels=3)
...
def build_depth_encoder(args):
    ...
    从 LightweightEncoder 创建，输入 1 通道（单通道深度图），
    输出 256 维特征图，空间尺寸为输入的 1/4。

    Args:
        args: 全局配置，目前 Depth 编码器不依赖额外参数。

    Returns:
        LightweightEncoder: 深度编码器实例。
    """
    from models.multimodal_encoders import LightweightEncoder
    return LightweightEncoder(in_channels=1)
