    ...
    const int im2col_step)
{
    ...
    TORCH_CHECK(value.type().is_cuda(), "value must be a CUDA tensor");
    TORCH_CHECK(spatial_shapes.type().is_cuda(), "spatial_shapes must be a CUDA tensor");
    TORCH_CHECK(level_start_index.type().is_cuda(), "level_start_index must be a CUDA tensor");
    TORCH_CHECK(sampling_loc.type().is_cuda(), "sampling_loc must be a CUDA tensor");
    TORCH_CHECK(attn_weight.type().is_cuda(), "attn_weight must be a CUDA tensor");
    ...
    const int im2col_step)
{
    ...
    TORCH_CHECK(sampling_loc.type().is_cuda(), "sampling_loc must be a CUDA tensor");
    TORCH_CHECK(attn_weight.type().is_cuda(), "attn_weight must be a CUDA tensor");
    TORCH_CHECK(grad_output.type().is_cuda(), "grad_output must be a CUDA tensor");
