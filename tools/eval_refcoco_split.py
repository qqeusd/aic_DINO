#!/usr/bin/env python3
"""Parallel RefCOCO eval: two GPUs, each handles half the data."""
import sys, os, json
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
import torch
from PIL import Image
from multiprocessing import Process, Queue
sys.path.insert(0, '.')
import datasets.transforms as T
from datasets.odvg import ODVGDataset
from models.registry import MODULE_BUILD_FUNCS
from util.slconfig import SLConfig
from groundingdino.util.utils import clean_state_dict

def eval_split(device_str, ckpt_path, desc, start_idx, end_idx, result_queue):
    device = torch.device(device_str)
    cfg = SLConfig.fromfile('config/cfg_refcoco_swinb.py')
    args = type('Args', (), {})()
    for k, v in cfg._cfg_dict.to_dict().items(): setattr(args, k, v)
    args.device = device_str
    args.coco_val_path = '/home/zlab8v100/ssd4/bigdata4/aic2026/coco2014/annotations/instances_val2014_5k.json'
    build_func = MODULE_BUILD_FUNCS.get('groundingdino')
    model, _, _ = build_func(args)
    ckpt = torch.load(ckpt_path, map_location='cpu')
    model.load_state_dict(clean_state_dict(ckpt['model']), strict=False)
    model = model.to(device).eval()

    ds = ODVGDataset(
        root='/home/zlab8v100/ssd4/bigdata4/aic2026/coco2014/train2014',
        anno='/home/zlab8v100/ssd4/bigdata4/aic2026/coco2014/refcoco/refcoco_val_odvg.jsonl',
    )

    transform = T.Compose([
        T.RandomResize([800], max_size=1333),
        T.ToTensor(),
        T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
    ])

    ious = []
    end_idx = min(end_idx, len(ds))
    for idx in range(start_idx, end_idx):
        meta = ds.metas[idx]
        img_path = os.path.join(ds.root, meta['filename'])
        image_pil = Image.open(img_path).convert('RGB')
        img_w, img_h = image_pil.size
        regions = meta['grounding']['regions']

        img_tensor, _ = transform(image_pil, None)
        img_tensor = img_tensor.to(device)

        for region in regions:
            gt_bbox = region['bbox']
            query = region['phrase'].lower().strip() + '.'
            gt_norm = [gt_bbox[0]/img_w, gt_bbox[1]/img_h, gt_bbox[2]/img_w, gt_bbox[3]/img_h]

            with torch.no_grad():
                outputs = model(img_tensor[None], captions=[query])

            logits = outputs['pred_logits'].sigmoid()[0]
            boxes = outputs['pred_boxes'][0]
            scores = logits.max(dim=1)[0]
            best = scores.argmax()
            b = boxes[best].tolist()
            cx, cy, w, h = b
            px1 = cx - w/2; py1 = cy - h/2
            px2 = cx + w/2; py2 = cy + h/2
            gx1, gy1, gx2, gy2 = gt_norm
            xi1, yi1 = max(px1, gx1), max(py1, gy1)
            xi2, yi2 = min(px2, gx2), min(py2, gy2)
            inter = max(0, xi2-xi1) * max(0, yi2-yi1)
            a1 = (px2-px1)*(py2-py1); a2 = (gx2-gx1)*(gy2-gy1)
            iou = inter / (a1 + a2 - inter + 1e-8)
            ious.append(iou)

        if (idx - start_idx + 1) % 300 == 0:
            acc05 = sum(1 for i in ious if i >= 0.5) / len(ious)
            print(f'  [{desc}] {idx+1}/{end_idx} acc@0.5={acc05:.4f}')

    acc05 = sum(1 for i in ious if i >= 0.5) / len(ious)
    print(f'\n[{desc}] DONE: {len(ious)} queries, Acc@0.5={acc05:.4f}')
    result_queue.put({'desc': desc, 'acc05': acc05, 'total': len(ious), 'ious': ious})

if __name__ == '__main__':
    ckpt1 = sys.argv[1] if len(sys.argv) > 1 else 'output/refcoco_all_swinb_baseline/checkpoint_best_regular.pth'
    ckpt2 = sys.argv[2] if len(sys.argv) > 2 else 'output/refcoco_all_swinb_baseline/checkpoint0005.pth'

    # Quick count
    ds = ODVGDataset(
        root='/home/zlab8v100/ssd4/bigdata4/aic2026/coco2014/train2014',
        anno='/home/zlab8v100/ssd4/bigdata4/aic2026/coco2014/refcoco/refcoco_val_odvg.jsonl',
    )
    N = len(ds)
    print(f'Total images: {N}, splitting: GPU1=best[0..{N//2}], GPU6=epoch5[{N//2}..{N}]')

    q = Queue()
    p1 = Process(target=eval_split, args=('cuda:1', ckpt1, 'best', 0, N//2, q))
    p2 = Process(target=eval_split, args=('cuda:6', ckpt2, 'epoch5', N//2, N, q))
    p1.start(); p2.start()
    p1.join(); p2.join()

    results = []
    while not q.empty():
        results.append(q.get())
    for r in results:
        print(f'\n=== [{r["desc"]}] Final: Acc@0.5={r["acc05"]:.4f} ({r["total"]} queries) ===')
