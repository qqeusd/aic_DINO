...
# [新增] 三模态 collate_fn — 将 dict-format samples 按 key 分别 stack
def collate_fn_multimodal(batch):
    """
    三模态数据的批处理函数，用于 DataLoader 的 collate_fn 参数。

    与标准 collate_fn 不同，每个 batch item 的 samples 是一个 dict
    （含 'rgb'/'ir'/'depth' 三个 key），需要分别 stack 而非
    通过 nested_tensor_from_tensor_list 拼接。

    Args:
        batch (List[dict]): 每个元素为 Multimodeldataset.__getitem__ 的返回值，
                            格式: {'rgb': [3,H,W], 'ir': [3,H,W], 'depth': [1,H,W], 'target': {...}}

    Returns:
        tuple:
            - samples (dict): {'rgb': [B,3,H,W], 'ir': [B,3,H,W], 'depth': [B,1,H,W]}
            - targets (List[dict]): 长度为 B 的 target 字典列表。

    Note:
        假定 batch 内所有图像已 resize 到统一尺寸，因此无需 padding。
        若后续启用多尺度训练，需在此处加入 NestedTensor 的 padding 逻辑。
    """
    samples = {}
